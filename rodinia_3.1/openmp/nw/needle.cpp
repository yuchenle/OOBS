...
void nw_optimized(int *input_itemsets, int *output_itemsets, int *referrence,
  ...
  printf("blk = 1, (max_cols-1)/BLOCK_SIZE is %d\n",
         (max_cols - 1) / BLOCK_SIZE);
#pragma omp target data map(to                                                 \
                            : max_cols, penalty, referrence [0:transfer_size]) \
    map(input_itemsets [0:transfer_size])
  {
    // outer loop added to see if cuda graph can be faster afterwards
    for (int iter = 0; iter < 1; ++iter) {
    
      long long inner_start_time = get_time();
#ifdef TDG
...
#pragma omp simd
        ...
        printf("blk = 2, (max_cols-1)/BLOCK_SIZE is %d\n",
               (max_cols - 1) / BLOCK_SIZE);
...
#pragma omp taskwait
      ...
      long long inner_end_time = get_time();
      printf("iteration %d took: %.3f seconds\n", iter,
            ((float)(inner_end_time - inner_start_time)) / (1000 * 1000));
